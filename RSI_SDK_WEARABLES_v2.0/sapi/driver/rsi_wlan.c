 ...
 * @param[in]    beacon_drop_threshold          - LMAC beacon drop threshold(ms): The amount of time that FW waits to receive full beacon. 
 *                                                 \n  Default value is 5000ms. 
 * @param[in]    filter_bcast_in_tim            - If this bit is set, then from the next dtim any broadcast data pending bit \n in TIM indicated will be ignored 
 *                                                  \n valid values: 0 - 1 
    ...
    if (rsi_socket_pool[i].sock_state > RSI_SOCKET_STATE_INIT) {
      ...
      if (rsi_wlan_cb_non_rom->socket_cmd_rsp_pending & BIT(i)) {
        ...
        if (rsi_wlan_cb_non_rom->callback_list.socket_connect_response_handler != NULL) {
          ...
          // Call asynchronous response handler to indicate to host
          rsi_wlan_cb_non_rom->callback_list.socket_connect_response_handler(RSI_ERROR_IN_WLAN_CMD,
                                                                             (uint8_t *)&socket_rsp,
 ...
 * @brief       By wireless library to acquire or wait for nwk semaphore.
...
void rsi_call_asynchronous_callback()
{
  if ((rsi_wlan_cb_non_rom->nwk_cmd_rsp_pending & (PING_RESPONSE_PENDING))
    ...
    // Call asynchronous response handler to indicate to host
    rsi_wlan_cb_non_rom->callback_list.wlan_ping_response_handler(RSI_ERROR_IN_WLAN_CMD, NULL, 0);
    rsi_wlan_cb_non_rom->nwk_cmd_rsp_pending &= ~(PING_RESPONSE_PENDING);
  }
  ...
  if ((rsi_wlan_cb_non_rom->nwk_cmd_rsp_pending & (DNS_RESPONSE_PENDING))
    ...
    rsi_wlan_cb_non_rom->nwk_callbacks.rsi_dns_update_rsp_handler(RSI_ERROR_IN_WLAN_CMD);
    rsi_wlan_cb_non_rom->nwk_cmd_rsp_pending &= ~(DNS_RESPONSE_PENDING);
  }
  if ((rsi_wlan_cb_non_rom->nwk_cmd_rsp_pending & (SNTP_RESPONSE_PENDING))
    ...
    rsi_wlan_cb_non_rom->nwk_callbacks.rsi_sntp_client_create_response_handler(RSI_ERROR_IN_WLAN_CMD, 0, NULL);
    rsi_wlan_cb_non_rom->nwk_cmd_rsp_pending &= ~(SNTP_RESPONSE_PENDING);
  }
