...
#endif
#if (RSI_ENABLE_PROP_PROTOCOL_TEST && WLAN_THROUGHPUT_TEST)
rsi_semaphore_handle_t prop_protocol_wlan_throughput_sync_sem;
#endif
...
bool rsi_ble_running, rsi_bt_running, rsi_wlan_running, rsi_prop_protocol_running, wlan_radio_initialized,
  powersave_cmd_given;
...
void rsi_common_app_task(void)
{
  ...
  while (1) {
    ...
    status = rsi_task_create((rsi_task_function_t)rsi_bt_a2dp_task,
                             (uint8_t *)"bt_a2dp_task",
                             RSI_BT_APP_TASK_SIZE + RSI_SBC_APP_ENCODE_SIZE,
                             NULL,
                             RSI_BT_APP_TASK_PRIORITY,
                             &bt_app_task_handle);
    if (status != RSI_ERROR_NONE) {
      LOG_PRINT("\r\n rsi_bt_a2dp_task failed to create \r\n");
      return;
    }
#if WLAN_THROUGHPUT_TEST
    ...
    status                    = rsi_semaphore_create(&prop_protocol_app_sem, 0);
    if (status != RSI_ERROR_NONE) {
      LOG_PRINT("\r\n failed to create prop_protocol_app_sem \r\n");
      return;
    }
#if WLAN_THROUGHPUT_TEST
    status = rsi_semaphore_create(&prop_protocol_wlan_throughput_sync_sem, 0);
    if (status != RSI_ERROR_NONE) {
      LOG_PRINT("\r\n failed to create prop_protocol_wlan_throughput_sync_sem \r\n");
      return;
    }
#endif
