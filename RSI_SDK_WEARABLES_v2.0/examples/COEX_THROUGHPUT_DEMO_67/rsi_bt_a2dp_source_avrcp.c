...
#define AVDTP_STATS_UPDATE_RATE \
  200 //! This will be considered as millisec. Minimum Time will be fine tunes by running all the possible modes.
...
#endif
extern bool rsi_bt_running, rsi_prop_protocol_running, rsi_wlan_running, wlan_radio_initialized, powersave_cmd_given;
...
int16_t send_audio_data(FILE *fp)
{
  ...
  rsi_mutex_unlock(&power_cmd_mutex);
#endif err = read_mp3_data_from_sd();
...
#if (RSI_ENABLE_BLE_TEST || RSI_ENABLE_PROP_PROTOCOL_TEST)
...
int32_t rsi_bt_a2dp_task()
{
  ...
  //! Measure the WLAN throughput after BT A2DP init
#if BT_A2DP_INIT_DONE
  ...
  while (1) {
    ...
    switch (temp_event_map) {
      ...
      case RSI_APP_EVENT_A2DP_CONN: {
        ...
        //! Measure the WLAN throughput after BT A2DP connection
      ...
      case RSI_APP_EVENT_A2DP_OPEN: {
        ...
        LOG_PRINT("\r\n GET_CONFIG: ChannelMode=%d | Freq=%d | ",
        ...
        set_sbc_cap.AllocMethod = ALLOCMETHOD; //SBC_ALLOCATION_LOUDNESS;
        set_sbc_cap.SubBands    = SUBBANDS;    //SBC_SUBBANDS_8;
        set_sbc_cap.BlockLength = BLOCKLENGTH; //SBC_BLOCK_LENGTH_16;
        set_sbc_cap.MaxBitPool  = MAXBITPOOL;  //35;
      ...
      case RSI_APP_EVENT_A2DP_START: {
        ...
        if (local_dev_role == RSI_MASTER_ROLE) //0 - Master role //1 - Slave Role
        {
          rsi_bt_app_clear_event(RSI_APP_EVENT_A2DP_START);
#if (RSI_AUDIO_DATA_TYPE == PCM_AUDIO && !TA_BASED_ENCODER)
          rsi_bt_app_set_event(RSI_APP_EVENT_A2DP_SBC_ENCODE);
...
#endif
          LOG_PRINT("\r\n a2dp send audio\n");
          //! Measure the WLAN throughput while audio is playing
#if (BT_A2DP_START && WLAN_THROUGHPUT_TEST)
          rsi_semaphore_post(&bt_wlan_throughput_sync_sem);
          //rsi_task_suspend(NULL);
#endif
        } else
