    ...
    switch (wlan_thrput_conf->throughput_type) {
      case UDP_TX: {
        ...
        if (client_socket < 0) {
          ...
          LOG_PRINT("\r\nSocket Create Failed, Error Code : 0x%X, conn%d\r\n", status, wlan_thrput_conf->thread_id);
          return status;
        } else {
        ...
        while (1) {
          ...
          if (status < 0) {
            ...
            rsi_free(send_buf);
            LOG_PRINT("\r\nFailed to send data to UDP Server, Error Code : 0x%X, conn%d\r\n",
                      status,
                      wlan_thrput_conf->thread_id);
          ...
          if (tt_end - tt_start >= THROUGHPUT_AVG_TIME) {
            ...
            LOG_PRINT("\r\nUDP tx throughput test completed, conn%d\r\n", wlan_thrput_conf->thread_id);
            //! Measure throughput
      ...
      case UDP_RX: {
        ...
        if (server_socket < 0) {
          ...
          LOG_PRINT("\r\nSocket Create Success, conn%d\r\n", wlan_thrput_conf->thread_id);
			...
			{
				LOG_PRINT("\r\nSet Socket Option Failed, Error Code : 0x%X, conn%d\r\n",status, wlan_thrput_conf->thread_id);
				return status;
			}
			else
			{
			...
			if(status != RSI_SUCCESS)
			{
				...
				LOG_PRINT("\r\nBind Failed, Error code : 0x%X, conn%d\r\n",status, wlan_thrput_conf->thread_id);
				return status;
			}
			else
			{
			...
			if(status != RSI_SUCCESS)
			{
				status = rsi_wlan_get_status();
				rsi_shutdown(server_socket, 0);
				LOG_PRINT("\r\nListen Failed, Error code : 0x%X, conn%d\r\n",status, wlan_thrput_conf->thread_id);
				return status;
			}
			else
			{
			...
			if(new_socket < 0)
			{
				status = rsi_wlan_get_status();
				rsi_shutdown(server_socket, 0);
				LOG_PRINT("\r\nSocket Accept Failed, Error code : 0x%X, conn%d\r\n",status, wlan_thrput_conf->thread_id);
				return status;
			}
			else
			{
      ...
      case TCP_TX: {
        ...
        if (status != RSI_SUCCESS) {
          status = rsi_wlan_get_status();
          //rsi_shutdown(client_socket, 0);
          LOG_PRINT("\r\nConnect to Server Socket Failed, Error Code : 0x%X, conn%d \r\n",
                    status,
                    wlan_thrput_conf->thread_id);
          //return status;
          continue;
        ...
        while (1) {
          ...
          if (status < 0) {
            ...
            rsi_shutdown(client_socket, 0);
            rsi_free(send_buf);
            LOG_PRINT("\r\nFailed to Send data to TCP Server, Error Code : 0x%X, conn%d \r\n",
                      status,
                      wlan_thrput_conf->thread_id);
            LOG_PRINT("\r\n client socket is shutdown, conn -%d \r\n", wlan_thrput_conf->thread_id);
            //rsi_wlan_app_cb.state = RSI_WLAN_IDLE_STATE;
      ...
      case SSL_TX: {
        ...
        if (status != RSI_SUCCESS) {
          //status = rsi_wlan_get_status();
          //rsi_shutdown(client_socket, 0);
          LOG_PRINT("\r\nFailed to Connect to SSL Server, Error code : 0x%X,conn%d\r\n",
                    status,
                    wlan_thrput_conf->thread_id);
          //return status;
          continue;
      ...
      case SSL_RX: {
        ...
        if (status != RSI_SUCCESS) {
          status = rsi_wlan_get_status();
          rsi_shutdown(client_socket, 0);
          LOG_PRINT("\r\nFailed to Connect to Server, Error code : 0x%X, conn%d\r\n",
                    status,
                    wlan_thrput_conf->thread_id);
          //return status;
          continue;
