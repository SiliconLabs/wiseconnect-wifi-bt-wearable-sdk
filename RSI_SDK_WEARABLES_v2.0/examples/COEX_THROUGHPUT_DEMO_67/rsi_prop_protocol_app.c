...
/*=======================================================================*/
//! Common include file
#define RSI_BLE_LOCAL_NAME  "PROP_PROTOCOL+RSI_COEX_MAX_DEMO"
...
extern rsi_semaphore_handle_t wlan_sync_coex_prop_protocol_sem;
uint8_t prop_protocol_stop = 0;
#endif
extern rsi_semaphore_handle_t ble_main_task_sem, ble_slave_conn_sem, bt_app_sem, prop_protocol_app_sem, wlan_app_sem,
  bt_inquiry_sem, ble_scan_sem;
#if WLAN_THROUGHPUT_TEST
extern rsi_semaphore_handle_t prop_protocol_wlan_throughput_sync_sem;
#endif
/*========================================================================*/
//!  CALLBACK FUNCTIONS
/*=======================================================================*/
...
void rsi_prop_protocol_task(void)
{
  ...
  if (disable_factor_count == DISABLE_ITER_COUNT) {
    //! Make prop_protocol disable.
    next_state         = STOP_PROP_PROTOCOL;
    bPROP_PROTOCOLTick = true;
  }
#endif
  //! Measure the WLAN throughput while data tranfer is in progress
#if PROP_PROTOCOL_DATA_TRANSFER_START
  //LOG_PRINT("\r\n PROP_PROTOCOL data transfer is in progress...............");
#if WLAN_THROUGHPUT_TEST
  rsi_semaphore_post(&prop_protocol_wlan_throughput_sync_sem);
#endif
#endif
#ifdef GET_PROP_PROTOCOL_STATS
  if (bStatsTick) {
...
#if WLAN_TRANSIENT_CASE
        if (rsi_wlan_running) {
          rsi_semaphore_post(&wlan_sync_coex_prop_protocol_sem);
          prop_protocol_stop = 1;
          printf("disabled prop_protocol activity\n");
        }
#endif
...
void rsi_prop_protocol_app_task(void)
{
  ...
  //! start BT inquiry after PROP_PROTOCOL and BLE init
#if RSI_ENABLE_BT_TEST
  //! start BLE scanning after PROP_PROTOCOL initialization
#if RSI_ENABLE_BLE_TEST
  rsi_semaphore_post(&ble_scan_sem);
  //! start BT inquiry after PROP_PROTOCOL init
#elif RSI_ENABLE_BT_TEST
  rsi_semaphore_post(&bt_inquiry_sem);
#endif
#endif

  //! Measure the WLAN throughput after PROP_PROTOCOL init
#if PROP_PROTOCOL_INIT_DONE
  LOG_PRINT("\r\n PROP_PROTOCOL activity completed...............");
#if WLAN_THROUGHPUT_TEST
  rsi_semaphore_post(&prop_protocol_wlan_throughput_sync_sem);
#endif
