...
#endif
extern bool rsi_ble_running, rsi_bt_running, rsi_prop_protocol_running, wlan_radio_initialized, powersave_cmd_given;
extern rsi_mutex_handle_t power_cmd_mutex;
#if (WLAN_THROUGHPUT_TEST && RSI_ENABLE_BT_TEST)
extern rsi_semaphore_handle_t bt_wlan_throughput_sync_sem;
#endif
#if (WLAN_THROUGHPUT_TEST && RSI_ENABLE_BLE_TEST)
extern rsi_semaphore_handle_t ble_wlan_throughput_sync_sem;
#endif
#if (WLAN_THROUGHPUT_TEST && RSI_ENABLE_PROP_PROTOCOL_TEST)
...
void compute_throughput(void)
{
  ...
  throughput = ((uint32_t)(num_bits) / xfer_time); //!Throughput calculation
  LOG_PRINT("\r\nThroughput in kbps : %d \r\n", throughput);
...
#else
  int8_t j = 0, retry = 0, retry_cnt = 0;
  ...
  while (1) {
    switch (rsi_wlan_app_cb.state) {
      ...
      case RSI_WLAN_IPCONFIG_DONE_STATE: {
#if WLAN_THROUGHPUT_TEST
#if WLAN_SYNC_REQ
        //! start wifi tx/rx after bt/ble and prop_protocol start-up activities
        ...
        rsi_semaphore_post(&sync_coex_prop_protocol_sem); // start PROP_PROTOCOL task init
        rsi_delay_ms(100);
#endif
#endif
...
#if RSI_ENABLE_PROP_PROTOCOL_TEST
        //LOG_PRINT("\r\n WLAN task waiting for BLE activity to complete.............");
        rsi_semaphore_wait(&prop_protocol_wlan_throughput_sync_sem, 0);
#endif
        ...
        for (uint8_t i = 0; i < socket_instances; i++) {
          ...
          status = rsi_task_create((void *)wlan_throughput_task,
...
#endif
#if HTTP_DOWNLOAD_THROUGPUT_TEST
        compute_throughput();
#endif

        for (uint8_t i = 0; i < SOCKTEST_INSTANCES_MAX; i++) {
