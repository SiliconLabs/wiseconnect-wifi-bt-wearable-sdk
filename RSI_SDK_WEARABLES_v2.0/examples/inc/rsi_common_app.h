...
#define BLE_MULTI_SLAVE_MASTER     0
...
#define COEX_MAX_APP_BLE_2MAS_8SLAV            0
#define COEX_THROUGHPUT                        0

#define PROP_PROTOCOL_CW_MODE 0
...
#endif
#define BLE_MULTI_SLAVE_MASTER_BT_A2DP_WIFI_HTTP_S_RX 0
...
#define PROP_PROTOCOL_APP                      0
#define BLE_CENTRAL_PROP_PROTOCOL_APP          0
#define BLE_PERIPHERAL_PROP_PROTOCOL_APP       0
#define PROP_PROTOCOL_BLE_PERI_BT_A2DP_SRC_APP 0
#define PROP_PROTOCOL_BLE_CENT_BT_A2DP_SRC_APP 0
...
#if COEX_MAX_APP_BLE_2MAS_8SLAV // Major
//! By default this macro is disabled
//! Enable this macro to take run time inputs
#define RUN_TIME_CONFIG_ENABLE 0
//! compile time configurations
...
#define RSI_COEX_MODE (0x109)
       ...
       || (BLE_ADV_BT_SPP_THROUGHPUT && (!BLE_ADVERTISE_ENABLE)) || BT_EIR_SNIFF_SPP_SLAVE || BT_PAGE_INQUIRY          \
       || BT_SNIFF || BT_PER || BT_SPP_RX_TX || BT_INQUIRY_SCAN || BT_PAGE_SCAN || BT_SPP_MASTER_SNIFF                 \
       || BT_SPP_SLAVE_SNIFF || BT_A2DP_SOURCE_WIFI_HTTP_S_RX_DYN_COEX || BT_CW_MODE)
#define RSI_COEX_MODE 5
#elif ((BLE_ADV_BT_SPP_THROUGHPUT && BLE_ADVERTISE_ENABLE) || BLE_DUAL_MODE_BT_A2DP_SOURCE_WIFI_HTTP_S_RX \
       || BLE_DUAL_MODE_BT_SPP_SLAVE || BLE_ADV_BT_A2DP_SOURCE || BT_BLE_IDLE_POWERSAVE                   \
       || BLE_MULTI_SLAVE_MASTER_BT_A2DP || BLE_MULTI_SLAVE_MASTER_BT_A2DP_WIFI_HTTP_S_RX || BT_BLE_UPDATE_GAIN_TABLE)
#define RSI_COEX_MODE 9
       ...
       || BLE_GATT_INDICATION_STATUS || BLE_SIMPLE_GATT || BLE_MULTI_SLAVE_MASTER || BLE_INDICATION                 \
       || BLE_OVERLAPPING_PROCEDURES || BLE_CW_MODE)
#define RSI_COEX_MODE 13
#elif (PROP_PROTOCOL_APP || BLE_CENTRAL_PROP_PROTOCOL_APP || BLE_PERIPHERAL_PROP_PROTOCOL_APP)
#define RSI_COEX_MODE (0x10D)
#elif (PROP_PROTOCOL_BLE_PERI_BT_A2DP_SRC_APP || PROP_PROTOCOL_BLE_CENT_BT_A2DP_SRC_APP     \
       || PROP_PROTOCOL_BLE_DUAL_ROLE_BT_A2DP_SRC_APP || WLAN_STANDBY_WITH_PROP_PROTOCOL    \
       || WIFI_PROP_PROTOCOL_HTTP_S_5MB_RX_WITH_STANDBY || UNIFIED_PROTOCOL || COEX_MAX_APP \
       || PROP_PROTOCOL_APP_POWERSAVE || PROP_PROTOCOL_CW_MODE || COEX_THROUGHPUT || COEX_MAX_APP_BLE_2MAS_8SLAV)
#define RSI_COEX_MODE (0x109)
#endif
...
int32_t rsi_app_task_wifi_prop_protocol_https_rx_standby(void);
...
void rsi_app_task_prop_protocol_app(void);
void rsi_prop_protocol(void);
int32_t rsi_prop_protocol_power_save(void);
#elif PROP_PROTOCOL_APP_POWERSAVE
void rsi_app_task_prop_protocol_app_power_save(void);
int32_t rsi_prop_protocol_power_save(void);
...
//  extern void rsi_prop_protocol_register_callbacks (rsi_prop_protocol_resp_handler_t   prop_protocol_async_resp_handler);
#elif PROP_PROTOCOL_BLE_PERI_BT_A2DP_SRC_APP
uint32_t rsi_ble_prop_protocol_app(void);
void rsi_ble_prop_protocol_app_task(void);
...
uint32_t rsi_ble_prop_protocol_app(void);
void rsi_ble_prop_protocol_app_task(void);
int32_t rsi_prop_protocol_ble_cent_bt_a2dp_source(void);
...
int32_t rsi_prop_protocol_ble_dual_role_bt_a2dp_source(void);
...
#elif RSI_ENABLE_PROP_PROTOCOL_TEST
int32_t rsi_prop_protocol_task(void);
#endif
#elif (WLAN_STANDBY_WITH_PROP_PROTOCOL)
void rsi_wlan_prop_protocol_common_task();
int32_t rsi_wlan_standby_with_prop_protocol_task(void);
int32_t rsi_prop_protocol_task(void);
void rsi_ble_prop_protocol_app_task(void);
#elif BT_CW_MODE
