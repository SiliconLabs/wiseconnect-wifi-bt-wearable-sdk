...
static volatile uint64_t slave2_state;
static rsi_ble_profile_list_by_conn_t rsi_ble_profile_list_by_conn[RSI_MAX_PROFILE_CNT] = { 0 };

uint8_t slave2_smp_done, slave2_mtu_done, slave2_smp_pairing_initated;
uint8_t slave_2_event_tracker;
uint8_t slave_2_received_notifications;
...
extern uint32_t rsi_s2_discnt_reason;
...
extern rsi_semaphore_handle_t ble_main_task_sem, ble_slave2_sem;
extern void rsi_ble_app_set_event(uint32_t event_num);

/*==============================================*/
/**
 * @fn         rsi_ble_slave2_app_set_event
 ...
 */
void rsi_ble_slave2_app_set_event(uint32_t event_num)
{
  if (event_num < 32) {
    ble_app_event_task_map[1] |= BIT(event_num);
  } else {
    ble_app_event_task_map1[1] |= BIT((event_num - 32));
  }
...
/*==============================================*/
/**
 * @fn         rsi_ble_slave2_app_clear_event
 ...
 */
static void rsi_ble_slave2_app_clear_event(uint32_t event_num)
{
  if (event_num < 32) {
    ble_app_event_task_map[1] &= ~BIT(event_num);
  } else {
...
int32_t rsi_ble_slave2_app_get_task_event(void)
{
  uint32_t ix;

  for (ix = 0; ix < 64; ix++) {
    if (ix < 32) {
      if (ble_app_event_task_map[1] & (1 << ix)) {
        return ix;
      }
    } else {
      if (ble_app_event_task_map1[1] & (1 << (ix - 32))) {
        return ix;
      }
    }
  }
...
void rsi_ble_slave2_app_task(void)
{
...
#if SMP_ENABLE_S2
  uint8_t slave2_remote_address[18] = { 0 };
#endif
  ...
  while (1) {
    ...
    event_id = rsi_ble_slave2_app_get_task_event();
    if (event_id == -1) {
      //! wait on events
      rsi_semaphore_wait(&ble_slave2_sem, 0);
      continue;
    }
    ...
    if (!((event_id == RSI_APP_EVENT_ADV_REPORT) || (event_id == RSI_BLE_CONN_EVENT)
      ...
      if (rsi_ble_conn_info[1].conn_status != 1) {
        //! Check if connection is done or not;
        rsi_ble_slave2_app_clear_event(event_id);
      }
    }

    switch (event_id) {
      case RSI_APP_EVENT_ADV_REPORT: {
        LOG_PRINT("\n Advertise report received -s2\n");
        //! clear the advertise report event.
        rsi_ble_slave2_app_clear_event(RSI_APP_EVENT_ADV_REPORT);
        ...
        status = rsi_ble_connect_with_params(rsi_ble_conn_info[1].rsi_app_adv_reports_to_app.dev_addr_type,
                                             (int8_t *)rsi_ble_conn_info[1].rsi_app_adv_reports_to_app.dev_addr,
        ...
        LOG_PRINT_D("Connecting to device : \ndev_addr : %s\n conn_int_max: %d \nconn_int_min: %d \ntimeout: %d -s2\n",
                    (int8_t *)rsi_ble_conn_info[1].remote_dev_addr,
                    M2S34_CONNECTION_INTERVAL_MAX,
                    M2S34_CONNECTION_INTERVAL_MIN,
                    M2S34_SUPERVISION_TIMEOUT);
#endif
        LOG_PRINT("\n connecting to device :  %s -s2 \n", (int8_t *)rsi_ble_conn_info[1].remote_dev_addr);

        if (status != RSI_SUCCESS) {
          LOG_PRINT("\n Connecting failed with status : 0x%x -s2\n", status);
          //return status;
          rsi_ble_app_set_event(RSI_BLE_SCAN_RESTART_EVENT);
        }

        rsi_semaphore_wait(&ble_slave2_sem, 5000);

        event_id = rsi_ble_slave2_app_get_task_event();

        if ((event_id == -1) || (!(event_id & (RSI_BLE_CONN_EVENT | RSI_BLE_ENHC_CONN_EVENT)))) {
          LOG_PRINT("\n Initiating connect cancel command in s2\n");
          status = rsi_ble_connect_cancel((int8_t *)rsi_ble_conn_info[1].rsi_app_adv_reports_to_app.dev_addr);
          if (status != RSI_SUCCESS) {
            LOG_PRINT("\n ble connect cancel cmd status = %x \n", status);
          } else {
            num_of_conn_slaves++;
            rsi_ble_slave2_app_set_event(RSI_BLE_DISCONN_EVENT);
          }
      ...
      case RSI_BLE_CONN_EVENT: {
        ...
        rsi_ble_slave2_app_clear_event(RSI_BLE_CONN_EVENT);
        rsi_ble_conn_info[1].conn_status = 1;

#if !CONNECTIVITY_TEST
        rsi_ble_slave2_app_set_event(RSI_BLE_REQ_GATT_PROFILE);
#endif
        num_of_conn_slaves++;

        LOG_PRINT("\n Number of slave devices connected:%d \n", num_of_conn_slaves);
        LOG_PRINT("\n Number of master devices connected:%d \n", num_of_conn_masters);
        memcpy(&rsi_connected_dev_addr, rsi_ble_conn_info[1].conn_event_to_app.dev_addr, RSI_DEV_ADDR_LEN);

        slave_2_event_tracker |= BIT(0); //! Setting the connected bit

#if 1
        if (RSI_BLE_MTU_EXCHANGE_FROM_HOST) {
          rsi_ascii_dev_address_to_6bytes_rev(rsi_connected_dev_addr, (int8_t *)rsi_ble_conn_info[1].remote_dev_addr);
        ...
        if (num_of_conn_slaves < RSI_BLE_MAX_NBR_SLAVES) {
          ...
          if (status != RSI_SUCCESS) {
            LOG_PRINT("\n scan channel failed to open 0x%x -s2\n", status);
            //continue;
          }
        }
      ...
      case RSI_BLE_ENHC_CONN_EVENT: {
        LOG_PRINT("\r\nIn on_enhance_conn evt - s2\r\n");
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_ENHC_CONN_EVENT);
        rsi_ble_conn_info[1].conn_status = 1;

        //slave2_state |= BIT(RSI_BLE_ENHC_CONN_EVENT);
#if !CONNECTIVITY_TEST
        rsi_ble_slave2_app_set_event(RSI_BLE_REQ_GATT_PROFILE);
#endif
        num_of_conn_slaves++;
        ...
        LOG_PRINT("\n Number of slave devices connected:%d \n", num_of_conn_slaves);
        LOG_PRINT("\n Number of master devices connected:%d \n", num_of_conn_masters);
        memcpy(rsi_connected_dev_addr, rsi_ble_conn_info[1].rsi_enhc_conn_status.dev_addr, RSI_DEV_ADDR_LEN);
        slave_2_event_tracker |= BIT(0); //! Setting the connected bit
      ...
      case RSI_BLE_MTU_EXCHANGE_INFORMATION: {
        rsi_ble_slave2_app_clear_event(RSI_BLE_MTU_EXCHANGE_INFORMATION);
        LOG_PRINT("\r\n MTU EXCHANGE INFORMATION - in subtask -s2 \r\n");
        if ((rsi_ble_conn_info[1].mtu_exchange_info.initiated_role == PEER_DEVICE_INITATED_MTU_EXCHANGE)
          ...
          if (status == RSI_ERROR_BLE_ATT_CMD_IN_PROGRESS) {
            slave2_state |= BIT64(RSI_BLE_MTU_EXCHANGE_INFORMATION);
            LOG_PRINT("\r\n rsi_ble_mtu_exchange_resp procedure is already in progress -s2 \r\n");
            break;
          }
          if (status != RSI_SUCCESS) {
      ...
      case RSI_BLE_MORE_DATA_REQ_EVT: {
        ...
        rsi_ble_slave2_app_clear_event(RSI_BLE_MORE_DATA_REQ_EVT);
        if (slave2_state & BIT64(RSI_DATA_TRANSFER_EVENT)) {
          slave2_state &= ~BIT64(RSI_DATA_TRANSFER_EVENT);
          rsi_ble_slave2_app_set_event(RSI_DATA_TRANSFER_EVENT);
        }
        if (slave2_state & BIT64(RSI_BLE_REQ_GATT_PROFILE)) {
          slave2_state &= ~BIT64(RSI_BLE_REQ_GATT_PROFILE);
          rsi_ble_slave2_app_set_event(RSI_BLE_REQ_GATT_PROFILE);
        }
        if (slave2_state & BIT64(RSI_BLE_GATT_PROFILES)) {
          slave2_state &= ~BIT64(RSI_BLE_GATT_PROFILES);
          rsi_ble_slave2_app_set_event(RSI_BLE_GATT_PROFILES);
        }
        if (slave2_state & BIT64(RSI_BLE_GATT_PROFILE)) {
          slave2_state &= ~BIT64(RSI_BLE_GATT_PROFILE);
          rsi_ble_slave2_app_set_event(RSI_BLE_GATT_PROFILE);
        }
        if (slave2_state & BIT64(RSI_BLE_GATT_CHAR_SERVICES)) {
          slave2_state &= ~BIT64(RSI_BLE_GATT_CHAR_SERVICES);
          rsi_ble_slave2_app_set_event(RSI_BLE_GATT_CHAR_SERVICES);
        }
        if (slave2_state & BIT64(RSI_BLE_READ_REQ_EVENT)) {
          slave2_state &= ~BIT64(RSI_BLE_READ_REQ_EVENT);
          rsi_ble_slave2_app_set_event(RSI_BLE_READ_REQ_EVENT);
        }
        if (slave2_state & BIT64(RSI_BLE_BUFF_CONF_EVENT)) {
          slave2_state &= ~BIT64(RSI_BLE_BUFF_CONF_EVENT);
          rsi_ble_slave2_app_set_event(RSI_BLE_BUFF_CONF_EVENT);
        }
        if (slave2_state & BIT64(RSI_BLE_GATT_WRITE_EVENT)) {
          slave2_state &= ~BIT64(RSI_BLE_GATT_WRITE_EVENT);
          rsi_ble_slave2_app_set_event(RSI_BLE_GATT_WRITE_EVENT);
        }
      ...
      case RSI_BLE_REQ_GATT_PROFILE: {
        if (slave2_mtu_done
          ...
          rsi_ble_slave2_app_clear_event(RSI_BLE_REQ_GATT_PROFILE);

          LOG_PRINT("\r\n remote device profile discovery started -s2 \r\n");
          status = rsi_ble_get_profiles_async(rsi_connected_dev_addr, 1, 0xffff, NULL);
          if (status != RSI_SUCCESS) {
            if (status == RSI_ERROR_BLE_ATT_CMD_IN_PROGRESS) {
              slave2_state |= BIT64(RSI_BLE_REQ_GATT_PROFILE);
              LOG_PRINT("\r\n rsi_ble_get_profiles_async procedure is already in progress -s2 \r\n");
              break;
            }
            //! check for buffer full error, which is not expected for this procedure
            else if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
              LOG_PRINT("\r\n rsi_ble_get_profiles_async failed with buffer full error -s2 \r\n");
              break;
            } else {
              LOG_PRINT("\r\n get profile async call failed with error code :%x -s2 \r\n", status);
            }
          }
        }
      } break;
      case RSI_BLE_GATT_PROFILES: {
        ...
        if (!prof_resp_recvd) {
          ...
          if (!done_profiles_query) {
            rsi_ble_slave2_app_clear_event(RSI_BLE_GATT_PROFILES);
            no_of_profiles = rsi_ble_conn_info[1].get_allprofiles.number_of_profiles;
            //! copy the end of handle of last searched profile
            profiles_endhandle =
              *(uint16_t *)(rsi_ble_conn_info[1].get_allprofiles.profile_desc[no_of_profiles - 1].end_handle);
            ...
            for (i = 0; i < no_of_profiles; i++) {
              memcpy(&rsi_ble_profile_list_by_conn[total_remote_profiles].profile_desc,
                     &rsi_ble_conn_info[1].get_allprofiles.profile_desc[i],
            ...
            if (profs_evt_cnt < total_remote_profiles) {
              ...
              if (status != RSI_SUCCESS) {
                ...
                if (status == RSI_ERROR_BLE_ATT_CMD_IN_PROGRESS) {
                  ...
                  slave2_state |= BIT64(RSI_BLE_GATT_PROFILES);
                  LOG_PRINT("\r\n rsi_ble_get_profile_async procedure is already in progress -s2 \r\n");
                  break;
                }
                //! check for buffer full error, which is not expected for this procedure
                else if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
            ...
            if (profile_index_for_char_query < total_remote_profiles) {
...
#if RSI_DEBUG_EN
              LOG_PRINT("\r\n search for profile characteristics :0x%x -s2 \r\n",
              ...
              if (status != RSI_SUCCESS) {
                ...
                if (status == RSI_ERROR_BLE_ATT_CMD_IN_PROGRESS) {
                  ...
                  slave2_state |= BIT64(RSI_BLE_GATT_PROFILE);
                  LOG_PRINT("\r\n rsi_ble_get_char_services_async procedure is already in progress -s2 \r\n");
                  break;
                }
                //! check for buffer full error, which is not expected for this procedure
                else if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
                  LOG_PRINT("\r\n rsi_ble_get_char_services_async failed with buffer full error -s2 \r\n");
                  break;
                } else {
                  LOG_PRINT(
                    "\r\n failed to get service characteristics of the remote GATT server with error:0x%x -s2 \r\n",
                    status);
                }
              }
        ...
        if (char_for_serv_cnt < total_remote_profiles) {
          ...
          for (uint8_t ix = 0; ix < rsi_ble_profile_list_by_conn[char_for_serv_cnt].no_of_char_services; ix++) {
            memcpy(&rsi_ble_profile_list_by_conn[char_for_serv_cnt].profile_char_info[ix],
                   &rsi_ble_conn_info[1].get_char_services,
            ...
            if (temp1 < total_remote_profiles) {
              ...
              if (temp2 < l_num_of_services) {
                ...
                if ((l_char_property == RSI_BLE_ATT_PROPERTY_INDICATE)
                  ...
                  //LOG_PRINT("\n query for profile service1 %d -s2 \n",temp1);
                  ...
                  if (status != RSI_SUCCESS) {
                    ...
                    if (status == RSI_ERROR_BLE_ATT_CMD_IN_PROGRESS) {
                      ...
                      slave2_state |= BIT64(RSI_BLE_GATT_CHAR_SERVICES);
                      LOG_PRINT("\r\n rsi_ble_get_att_value_async procedure is already in progress -s2 \r\n");
                      break;
                    }
                    //! check for buffer full error, which is not expected for this procedure
                    else if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
                      LOG_PRINT("\r\n rsi_ble_get_att_value_async failed with buffer full error -s2 \r\n");
                      break;
                    } else {
          ...
          //! discovering completed for all profiles
            ...
            rsi_ble_slave2_app_clear_event(RSI_BLE_GATT_CHAR_SERVICES);
            rsi_ble_slave2_app_set_event(RSI_BLE_GATT_DESC_SERVICES);
          }
        }
      } break;
      case RSI_BLE_GATT_DESC_SERVICES: {
        if (temp1 < total_remote_profiles) {
          temp2++;
          rsi_ble_slave2_app_clear_event(RSI_BLE_GATT_DESC_SERVICES);
          char_desc_cnt++;
          char_desc_resp_recvd = false;
        } else {
          rsi_ble_slave2_app_clear_event(RSI_BLE_GATT_DESC_SERVICES);
          LOG_PRINT("\n remote device profiles discovery completed -s2 \n");
          ...
          if (!indication_handle_found) {
            LOG_PRINT("\n Health Thermometer profile not found -s2 \n");
          }
            ...
            if (rsi_ble_profile_list_by_conn[i].profile_desc.profile_uuid.val.val16 == 0x1802) {
              for (uint8_t ix = 0; ix < rsi_ble_profile_list_by_conn[i].no_of_char_services; ix++) {
                if ((!write_wwr_handle_found)
...
#if RSI_DEBUG_EN
                  LOG_PRINT_D("\n write without response handle found -s2 \n");
#endif
                  ...
                  rsi_ble_slave2_app_set_event(RSI_BLE_BUFF_CONF_EVENT);
                  slave_2_event_tracker |= BIT(2); //! Trigger Write without response
                  break;
                }
              }
            }
          ...
          if (!write_wwr_handle_found) {
            LOG_PRINT("\n Immediate Alert profile not found -s2 \n");
          }
#elif CHECK_WRITE_PROPERTY
          ...
          if (!write_handle_found) {
            LOG_PRINT("\n heart rate profile not found -s2 \n");
          }
#elif CHECK_NOTIFICATIONS
          // check for heart rate profile
          for (i = 0; i < total_remote_profiles; i++) {
      ...
      case RSI_BLE_BUFF_CONF_EVENT: {
#if RSI_DEBUG_EN
        LOG_PRINT_D("\r\n in gatt RSI_BLE_BUFF_CONF_EVENT -s2 \r\n");
#endif
        ...
        if (status != RSI_SUCCESS) {
          LOG_PRINT("\r\n failed to set the buffer configuration error:0x%x -s2 \r\n", status);
        } else {
          rsi_ble_slave2_app_clear_event(RSI_BLE_BUFF_CONF_EVENT);
          rsi_ble_slave2_app_set_event(RSI_CONN_UPDATE_REQ_EVENT);
        }
      } break;
      case RSI_CONN_UPDATE_REQ_EVENT: {
#if RSI_DEBUG_EN
        LOG_PRINT_D("\r\n in connection update req event -s2 \r\n");
#endif
        ...
        if (status != RSI_SUCCESS) {
          //! check for procedure already in progress error
          if (status == RSI_ERROR_BLE_ATT_CMD_IN_PROGRESS) {
            LOG_PRINT("\r\n rsi_ble_conn_params_update procedure is already in progress -s2 \r\n");
            break;
          } else {
            LOG_PRINT("\r\n failed to update connection paramaters error:0x%x -s2 \r\n", status);
          }
        } else {
          rsi_ble_slave2_app_clear_event(RSI_CONN_UPDATE_REQ_EVENT);
          rsi_ble_slave2_app_set_event(RSI_DATA_TRANSFER_EVENT);
          LOG_PRINT("\n connection params request was successfull -s2 \n");
        }
      } break;
      ...
      case RSI_BLE_RECEIVE_REMOTE_FEATURES: {
        ...
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_RECEIVE_REMOTE_FEATURES);
#if SMP_ENABLE_S2
        if (slave2_mtu_done) {
          if (!slave2_smp_pairing_initated) {
            ...
            if (status != RSI_SUCCESS) {
              LOG_PRINT("\r\n start of SMP pairing process failed with error code %x -s2 \r\n", status);
            } else {
              slave2_smp_pairing_initated = 1;
              rsi_6byte_dev_address_to_ascii(slave2_remote_address, rsi_connected_dev_addr);
              LOG_PRINT("\r\n smp pairing request initiated to %s - s1\r\n", slave2_remote_address);
            }
          }
        }
#endif
      ...
      case RSI_BLE_CONN_UPDATE_COMPLETE_EVENT: {
        ...
        rsi_ble_slave2_app_clear_event(RSI_BLE_CONN_UPDATE_COMPLETE_EVENT);

        LOG_PRINT("\nconn updated device address : %s\nconn_interval:%d\ntimeout:%d -s2",
                  rsi_ble_conn_info[1].remote_dev_addr,
                  rsi_ble_conn_info[1].conn_update_resp.conn_interval,
                  rsi_ble_conn_info[1].conn_update_resp.timeout);
      ...
      case RSI_BLE_DISCONN_EVENT: {
        ...
        LOG_PRINT("\r\n slave2 is disconnected, reason : 0x%x -s2 \r\n", rsi_s2_discnt_reason);

        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_DISCONN_EVENT);

        rsi_ble_slave2_app_clear_event(RSI_DATA_TRANSFER_EVENT);

        ble_app_event_task_map[1]  = 0;
        ble_app_event_task_map1[1] = 0;

        rsi_remove_ble_conn_id(rsi_ble_conn_info[1].remote_dev_addr);

        slave2_smp_done              = 0;
        slave2_mtu_done              = 0;
        slave2_smp_pairing_initated  = 0;
        slave_2_event_tracker        = 0;
        prof_resp_recvd              = false;
        ...
        memset(&rsi_ble_conn_info[1], 0, sizeof(rsi_ble_conn_info_t));
        ...
        if (num_of_conn_slaves < RSI_BLE_MAX_NBR_SLAVES) {
          ...
          rsi_ble_slave2_app_set_event(RSI_BLE_SCAN_RESTART_EVENT);
        }
      } break;
      case RSI_BLE_GATT_WRITE_EVENT: {
        ...
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_GATT_WRITE_EVENT);

        LOG_PRINT("\r\nIn GATT Write evt - s2\r\n");
#if CHECK_INDICATIONS
        //! Check whether received write is indication and acknowledge it
        if (*(uint16_t *)rsi_ble_conn_info[1].app_ble_write_event.handle == (indication_handle)) {
#if RSI_BLE_INDICATE_CONFIRMATION_FROM_HOST
          //! Send indication acknowledgement to remote device
          status = rsi_ble_indicate_confirm(rsi_connected_dev_addr);
          if (status != RSI_SUCCESS) {
            LOG_PRINT("\n indication confirm failed \t reason = %x -s2\n", status);
          } else {
            LOG_PRINT("\n indication confirm response sent -s2\n");
          }
#endif
        }
#endif
#if CHECK_NOTIFICATIONS
        if (slave_2_event_tracker
            & BIT(0)) //! update connection parameter after receiving 3 notifications from remote device
        {
          LOG_PRINT("\n receiving notifications -s2\n");
        }
#endif
      } break;
      case RSI_DATA_TRANSFER_EVENT: {
#if CHECK_INDICATIONS
        LOG_PRINT("\n in indication event -s2 \n");
        rsi_ble_slave2_app_clear_event(RSI_DATA_TRANSFER_EVENT);
        ...
        if (status != RSI_SUCCESS) {
          if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
            LOG_PRINT("\r\n indication failed with buffer error -s2 \r\n");
            slave2_state |= BIT64(RSI_DATA_TRANSFER_EVENT);
            break;
          } else {
            LOG_PRINT("\r\n indication failed with status = %x -s2 \r\n", status);
          }
        }
...
#if RSI_DEBUG_EN
        LOG_PRINT_D("\n in write with response event -s2 \n");
#endif
        ...
        if (status != RSI_SUCCESS) {
          ...
          if (status == RSI_ERROR_BLE_ATT_CMD_IN_PROGRESS) {
            slave2_state |= BIT64(RSI_DATA_TRANSFER_EVENT);
#if RSI_DEBUG_EN
            LOG_PRINT_D("\r\n rsi_ble_set_att_value_async procedure is already in progress -s2 \r\n");
#endif
            break;
          } else if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
            LOG_PRINT("\r\n write with response failed with buffer error -s2 \r\n");
            slave2_state |= BIT64(RSI_DATA_TRANSFER_EVENT);
            break;
          } else {
            LOG_PRINT("\r\n write with response failed with status = %x -s2 \r\n", status);
          }
        } else {
          write_cnt++;
        }
        ...
        if (status != RSI_SUCCESS) {
          if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
            LOG_PRINT("\r\n notify failed with buffer error -s2 \r\n");
            slave2_state |= BIT64(RSI_DATA_TRANSFER_EVENT);
            break;
          } else {
            LOG_PRINT("\r\n notify value failed with status = %x -s2 \r\n", status);
          }
        }

        slave_2_event_tracker |= BIT(1); //Enabled notifications for remote device.
#elif CHECK_WRITE_WITHOUT_RESP
        read_data1[0]       = wwr_count;
#if RSI_DEBUG_EN
        LOG_PRINT_D("\n in write without response event -s2 \n");
#endif
        status = rsi_ble_set_att_cmd(rsi_connected_dev_addr, write_wwr_handle, 1, (uint8_t *)read_data1);
        if (status != RSI_SUCCESS) {
          if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
#if RSI_DEBUG_EN
            LOG_PRINT_D("\r\n write without response failed with buffer error -s2 \r\n");
#endif
            slave2_state |= BIT64(RSI_DATA_TRANSFER_EVENT);
            break;
          } else if (status == RSI_ERROR_IN_BUFFER_ALLOCATION) //! TO-DO, add proper error code
          {
            LOG_PRINT("\n cannot transmit %d bytes in small buffer configuration mode -s2\n", RSI_BLE_MAX_DATA_LEN);
            rsi_ble_disconnect(rsi_connected_dev_addr);
            break;
          } else {
            LOG_PRINT("\r\n write without response failed with status = 0x%x -s2 \r\n", status);
          }
          ...
          if (wwr_count == 3) {
            LOG_PRINT("\n start remote device profiles discovery again -s2 \n");
      ...
      case RSI_BLE_WRITE_EVENT_RESP: {
        ...
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_WRITE_EVENT_RESP);
#if CHECK_WRITE_PROPERTY
        rsi_ble_slave2_app_set_event(RSI_DATA_TRANSFER_EVENT);
        LOG_PRINT("\n write response received -s2\n");
#endif
      } break;
      case RSI_BLE_READ_REQ_EVENT: {
        ...
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_READ_REQ_EVENT);

        LOG_PRINT("\r\nIn GATT Read evt - s2\r\n");

        type   = rsi_ble_conn_info[1].app_ble_read_event.type;
        handle = rsi_ble_conn_info[1].app_ble_read_event.handle;
        offset = rsi_ble_conn_info[1].app_ble_read_event.offset;
        ...
        if (status != RSI_SUCCESS) {
          LOG_PRINT("\r\n read response failed, error:0x%x -s2 \r\n", status);
        } else {
          LOG_PRINT("\n response to read request initiated by remote device was successfull -s2 \n");
        }
      } break;
      ...
      case RSI_BLE_MTU_EVENT: {
        ...
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_MTU_EVENT);

        slave2_mtu_done = 1;
        LOG_PRINT("\r\nIn MTU evt - s2\r\n");
#if SMP_ENABLE_S2
        rsi_ble_slave2_app_set_event(RSI_BLE_RECEIVE_REMOTE_FEATURES);
#endif
      } break;
      case RSI_BLE_SCAN_RESTART_EVENT: {
        LOG_PRINT("\r\nIn Scan Re-Start evt - s2\r\n");
        rsi_ble_slave2_app_clear_event(RSI_BLE_SCAN_RESTART_EVENT);
        status = rsi_ble_stop_scanning();
        if (status != RSI_SUCCESS) {
          LOG_PRINT("\n scanning stop cmd status = %x -s2\n", status);
        }
        ...
        status = rsi_ble_start_scanning();
        if (status != RSI_SUCCESS) {
          LOG_PRINT("\n scanning start cmd status = %x -s2\n", status);
        }
      } break;
      case RSI_APP_EVENT_REMOTE_CONN_PARAM_REQ: {
        //! remote device conn params request
        //! clear the conn params request event.
        rsi_ble_slave2_app_clear_event(RSI_APP_EVENT_REMOTE_CONN_PARAM_REQ);

        LOG_PRINT("\r\nIn rem conn param req evt - s2\r\n");
        ...
        LOG_PRINT("\tCONN INTERVAL MIN:0x%04x\n",
                  rsi_ble_conn_info[1].rsi_app_remote_device_conn_params.conn_interval_min);
        LOG_PRINT("\tCONN INTERVAL MAX:0x%04x\n",
                  rsi_ble_conn_info[1].rsi_app_remote_device_conn_params.conn_interval_max);
        LOG_PRINT("\tCONN LATENCY:0x%04x\n", rsi_ble_conn_info[1].rsi_app_remote_device_conn_params.conn_latency);
#if RSI_DEBUG_EN
        LOG_PRINT_D("\tSUPERVISION TIMEOUT:0x%04x\n", rsi_ble_conn_info[1].rsi_app_remote_device_conn_params.timeout);
#endif
#if SMP_ENABLE_S2
        if (slave2_smp_done) {
          status =
            rsi_ble_conn_param_resp((uint8_t *)rsi_ble_conn_info[1].rsi_app_remote_device_conn_params.dev_addr, 0);
        } else {
          status =
            rsi_ble_conn_param_resp((uint8_t *)rsi_ble_conn_info[1].rsi_app_remote_device_conn_params.dev_addr, 0);
        }
#else
        status = rsi_ble_conn_param_resp((uint8_t *)rsi_ble_conn_info[1].rsi_app_remote_device_conn_params.dev_addr, 0);
#endif
        if (status != RSI_SUCCESS) {
      ...
      case RSI_BLE_GATT_ERROR: {
        rsi_ble_slave2_app_clear_event(RSI_BLE_GATT_ERROR);
        if ((*(uint16_t *)rsi_ble_conn_info[1].rsi_ble_gatt_err_resp.error) == RSI_END_OF_PROFILE_QUERY) {
          ...
          LOG_PRINT("\nGATT ERROR REASON:0x%x -s2 \n", *(uint16_t *)rsi_ble_conn_info[1].rsi_ble_gatt_err_resp.error);
        }
      } break;
      case RSI_BLE_CONN_UPDATE_REQ: {
        rsi_ble_slave2_app_clear_event(RSI_BLE_CONN_UPDATE_REQ);

        LOG_PRINT("\r\n Sending connection update - s2 \r\n");

        status = rsi_ble_conn_params_update(rsi_connected_dev_addr, 240, 240, 0, 400);
        if (status != RSI_SUCCESS) {
          LOG_PRINT("\r\n connection update failed -s2 \r\n");
        }
      } break;
      case RSI_APP_EVENT_DATA_LENGTH_CHANGE: {
        //! clear the disconnected event.
        rsi_ble_slave2_app_clear_event(RSI_APP_EVENT_DATA_LENGTH_CHANGE);

        if (rsi_ble_conn_info[1].remote_dev_feature.remote_features[1] & 0x01) {
          status = rsi_ble_setphy((int8_t *)rsi_ble_conn_info[1].remote_dev_feature.dev_addr,
                                  TX_PHY_RATE,
                                  RX_PHY_RATE,
                                  CODDED_PHY_RATE);
          if (status != RSI_SUCCESS) {
      ...
      case RSI_APP_EVENT_PHY_UPDATE_COMPLETE: {
        ...
        rsi_ble_slave2_app_clear_event(RSI_APP_EVENT_PHY_UPDATE_COMPLETE);
      } break;

      case RSI_BLE_SMP_REQ_EVENT: {
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_SMP_REQ_EVENT);

        LOG_PRINT("\r\n in smp request \r\n -s2 \r\n");
#if SMP_ENABLE_S2
        //! initiating the SMP pairing process
        status = rsi_ble_smp_pair_request(rsi_ble_conn_info[1].rsi_ble_event_smp_req.dev_addr,
      ...
      case RSI_BLE_SMP_RESP_EVENT: {
        ...
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_SMP_RESP_EVENT);

        LOG_PRINT("\r\n in smp response -s2 \r\n");
#if SMP_ENABLE_S2
        //! initiating the SMP pairing process
        status = rsi_ble_smp_pair_response(rsi_ble_conn_info[1].rsi_ble_event_smp_resp.dev_addr,
                                           RSI_BLE_SMP_IO_CAPABILITY,
                                           MITM_ENABLE);
#endif
      } break;
      ...
      case RSI_BLE_SMP_PASSKEY_EVENT: {
        ...
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_SMP_PASSKEY_EVENT);
#if RSI_DEBUG_EN
        LOG_PRINT_D("\r\n in smp passkey -s2 \r\n");
#endif
        ...
        //! initiating the SMP pairing process
        status = rsi_ble_smp_passkey(rsi_ble_conn_info[1].rsi_ble_event_smp_passkey.dev_addr, RSI_BLE_APP_SMP_PASSKEY);
      } break;
      case RSI_BLE_SMP_PASSKEY_DISPLAY_EVENT: {
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_SMP_PASSKEY_DISPLAY_EVENT);
#if RSI_DEBUG_EN
        LOG_PRINT_D("\r\n in smp pass key display -s2 \r\n");
#endif

        LOG_PRINT("\nremote addr: %s, passkey: %s \r\n",
                  rsi_ble_conn_info[1].remote_dev_addr,
                  rsi_ble_conn_info[1].rsi_ble_smp_passkey_display.passkey);
      } break;
      case RSI_BLE_LTK_REQ_EVENT: {
        //! event invokes when disconnection was completed

        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_LTK_REQ_EVENT);
#if RSI_DEBUG_EN
        LOG_PRINT_D("\r\n in LTK  request -s2 \r\n");
#endif

        if (0) /*((rsi_ble_conn_info[1].rsi_le_ltk_resp.localediv == l_rsi_encryption_enabled.localediv)
					&& !((memcmp(rsi_ble_conn_info[1].rsi_le_ltk_resp.localrand,
							l_rsi_encryption_enabled.localrand, 8))))*/
        {
          LOG_PRINT("\n positive reply\n");
          //! give le ltk req reply cmd with positive reply
          status =
            rsi_ble_ltk_req_reply(rsi_ble_conn_info[1].rsi_le_ltk_resp.dev_addr, 1, l_rsi_encryption_enabled.localltk);
          if (status != RSI_SUCCESS) {
            LOG_PRINT("\r\n failed to restart smp pairing with status: 0x%x -s2\r\n", status);
          }
        } else {
          LOG_PRINT("\n negative reply\n");
          //! give le ltk req reply cmd with negative reply
          status = rsi_ble_ltk_req_reply(rsi_ble_conn_info[1].rsi_le_ltk_resp.dev_addr, 0, NULL);
          if (status != RSI_SUCCESS) {
      ...
      case RSI_BLE_SC_PASSKEY_EVENT: {
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_SC_PASSKEY_EVENT);
#if RSI_DEBUG_EN
        LOG_PRINT_D("\r\n in smp sc passkey event -s2 \r\n");
#endif

        LOG_PRINT("\n In passkey event, remote addr: %s, passkey: %06d -s2 \r\n",
                  rsi_ble_conn_info[1].remote_dev_addr,
                  rsi_ble_conn_info[1].rsi_event_sc_passkey.passkey);

        rsi_ble_smp_passkey(rsi_ble_conn_info[1].rsi_event_sc_passkey.dev_addr,
                            rsi_ble_conn_info[1].rsi_event_sc_passkey.passkey);
      } break;
      ...
      case RSI_BLE_SECURITY_KEYS_EVENT: {
        ...
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_SECURITY_KEYS_EVENT);

        LOG_PRINT("\r\n in smp security keys event  -s2 \r\n");
      } break;
      ...
      case RSI_BLE_SMP_FAILED_EVENT: {
        ...
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_SMP_FAILED_EVENT);

        LOG_PRINT("\r\n in smp failed remote address: %s -s2 \r\n", rsi_ble_conn_info[1].remote_dev_addr);
      }
      ...
      case RSI_BLE_ENCRYPT_STARTED_EVENT: {
        ...
        slave2_smp_done = 1;
        //! clear the served event
        rsi_ble_slave2_app_clear_event(RSI_BLE_ENCRYPT_STARTED_EVENT);

        memcpy(&l_rsi_encryption_enabled,
               &rsi_ble_conn_info[1].rsi_encryption_enabled,
               sizeof(rsi_bt_event_encryption_enabled_t));

        LOG_PRINT("\r\n in smp encrypt event -s2 \r\n");
      } break;
