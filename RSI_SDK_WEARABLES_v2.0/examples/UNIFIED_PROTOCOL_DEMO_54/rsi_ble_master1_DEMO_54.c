/*******************************************************************************
* @file  rsi_ble_master1_DEMO_54.c
* @brief 
*******************************************************************************
* # License
...
uint8_t master_1_event_tracker, master1_smp_done, master1_mtu_done;
#if SMP_ENABLE_M1
static uint8_t first_connect = 0;
#endif
static volatile uint64_t master1_state;
 ...
 */
void rsi_ble_master1_app_set_event(uint32_t event_num)
{
  if (event_num < 32) {
    ble_app_event_task_map[RSI_BLE_MAX_NBR_SLAVES] |= BIT(event_num);
  } else {
    ble_app_event_task_map1[RSI_BLE_MAX_NBR_SLAVES] |= BIT((event_num - 32));
  }
...
/*==============================================*/
/**
 * @fn         rsi_ble_master1_app_clear_event
 ...
 */
static void rsi_ble_master1_app_clear_event(uint32_t event_num)
{
  if (event_num < 32) {
    ble_app_event_task_map[RSI_BLE_MAX_NBR_SLAVES] &= ~BIT(event_num);
  } else {
...
int32_t rsi_ble_master1_app_get_task_event(void)
{
  ...
  for (ix = 0; ix < 64; ix++) {
    if (ix < 32) {
      if (ble_app_event_task_map[RSI_BLE_MAX_NBR_SLAVES] & (1 << ix)) {
        return ix;
      }
    } else {
      if (ble_app_event_task_map1[RSI_BLE_MAX_NBR_SLAVES] & (1 << (ix - 32))) {
        return ix;
      }
    }
  }
...
void rsi_ble_master1_app_task(void)
{
  ...
  while (1) {
    ...
    event_id = rsi_ble_master1_app_get_task_event();
    if (event_id == -1) {
      //! wait on events
      rsi_semaphore_wait(&ble_master1_sem, 0);
      continue;
    }
    ...
    if (!((event_id == RSI_BLE_CONN_EVENT) || (event_id == RSI_BLE_ENHC_CONN_EVENT))) {
      if (rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].conn_status != 1) {
        //! Check if connection is done or not;
        rsi_ble_master1_app_clear_event(event_id);
      }
    }
    ...
    switch (event_id) {
      case RSI_BLE_CONN_EVENT: {
        ...
        rsi_ble_master1_app_clear_event(RSI_BLE_CONN_EVENT);
        rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].conn_status = 1;

#if !CONNECTIVITY_TEST
        rsi_ble_master1_app_set_event(RSI_BLE_REQ_GATT_PROFILE);
#endif
        num_of_conn_masters++;

        LOG_PRINT("\r\n Number of master devices connected:%d -m1\n", num_of_conn_masters);

        memcpy(rsi_connected_dev_addr,
               rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].conn_event_to_app.dev_addr,
               RSI_DEV_ADDR_LEN);
        ...
        if (status != RSI_SUCCESS) {
          LOG_PRINT("\n Failed to clear the white list, error: 0x%x -m1\n", status);
          break;
        }
        ...
        if (status != RSI_SUCCESS) {
          LOG_PRINT("\n Failed to add the device:%s to whitelist -m1\n", str_remote_address);
          break;
        }
        LOG_PRINT("\n device %s added to whitelist -m1 \n", str_remote_address);

#endif
        if (num_of_conn_masters < RSI_BLE_MAX_NBR_MASTERS) {
          status = rsi_ble_start_advertising();
          if (status != RSI_SUCCESS) {
            LOG_PRINT("\r\n advertising failed with status = 0x%x -m1 \n", status);
            //continue;
          }
        } else if (num_of_conn_masters == RSI_BLE_MAX_NBR_MASTERS) {
          status = rsi_ble_start_advertising_with_values(&change_adv_param);
          if (status != RSI_SUCCESS) {
            LOG_PRINT("\r\n advertising with values failed with status = 0x%x -m1 \n", status);
            //continue ;
          }
        }
        LOG_PRINT("\n advertising device -m1\n");
#if 1
      ...
      case RSI_BLE_ENHC_CONN_EVENT: {
        ...
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_ENHC_CONN_EVENT);
        rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].conn_status = 1;
#if !CONNECTIVITY_TEST
        rsi_ble_master1_app_set_event(RSI_BLE_REQ_GATT_PROFILE);
#endif
        ...
        memcpy(rsi_connected_dev_addr,
               rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_enhc_conn_status.dev_addr,
               RSI_DEV_ADDR_LEN);
        ...
        status = rsi_ble_clear_whitelist();
        if (status != RSI_SUCCESS) {
          LOG_PRINT("\n Failed to clean the whitelist , error: 0x%x \n", status);
        }
        status = rsi_ble_addto_whitelist((int8_t *)rsi_connected_dev_addr, LE_RANDOM_ADDRESS);
        rsi_6byte_dev_address_to_ascii(str_remote_address, rsi_connected_dev_addr);
        if (status != RSI_SUCCESS) {
      ...
      case RSI_BLE_MTU_EXCHANGE_INFORMATION: {
        rsi_ble_master1_app_clear_event(RSI_BLE_MTU_EXCHANGE_INFORMATION);
        LOG_PRINT("\r\n MTU EXCHANGE INFORMATION - in subtask -m1 \r\n");
        if ((rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].mtu_exchange_info.initiated_role
      ...
      case RSI_BLE_MORE_DATA_REQ_EVT: {
        ...
        rsi_ble_master1_app_clear_event(RSI_BLE_MORE_DATA_REQ_EVT);
#if RSI_DEBUG_EN
        LOG_PRINT_D("\r\n more data request -m1 \r\n");
#endif

        if (master1_state & BIT64(RSI_DATA_TRANSFER_EVENT)) {
          master1_state &= ~BIT64(RSI_DATA_TRANSFER_EVENT);
          rsi_ble_master1_app_set_event(RSI_DATA_TRANSFER_EVENT);
        }
      ...
      case RSI_BLE_REQ_GATT_PROFILE: {
        if (master1_mtu_done
          ...
          //
          rsi_ble_master1_app_clear_event(RSI_BLE_REQ_GATT_PROFILE);
          ...
          if (status != RSI_SUCCESS) {
            ...
            if (status == RSI_ERROR_BLE_ATT_CMD_IN_PROGRESS) {
              master1_state |= BIT64(RSI_BLE_REQ_GATT_PROFILE);
              LOG_PRINT("\r\n rsi_ble_get_profiles_async procedure is already in progress -m1 \r\n");
              break;
            }
          ...
          //! Enable this to check support of an error code, if a procedure is called while it is already in progress
        }
      } break;
      case RSI_BLE_GATT_PROFILES: {
        ...
        if (!prof_resp_recvd) {
          ...
          if (!done_profiles_query) {
            ...
            LOG_PRINT_D("\n in profile query -m1 \n");
#endif
            rsi_ble_master1_app_clear_event(RSI_BLE_GATT_PROFILES);
            no_of_profiles = rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].get_allprofiles.number_of_profiles;
            //! copy the end of handle of last searched profile
            profiles_endhandle = *(uint16_t *)(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES]
                                                 .get_allprofiles.profile_desc[no_of_profiles - 1]
                                                 .end_handle);
#if RSI_DEBUG_EN
            LOG_PRINT_D("\n profiles end handle = 0x%x\n", profiles_endhandle);
#endif
            ...
            for (i = 0; i < no_of_profiles; i++) {
              memcpy(&rsi_ble_profile_list_by_conn[total_remote_profiles].profile_desc,
                     &rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].get_allprofiles.profile_desc[i],
            ...
            if (profs_evt_cnt < total_remote_profiles) {
              ...
              if (status != RSI_SUCCESS) {
                ...
                if (status == RSI_ERROR_BLE_ATT_CMD_IN_PROGRESS) {
                  ...
                  master1_state |= BIT64(RSI_BLE_GATT_PROFILES);
#if RSI_DEBUG_EN
                  LOG_PRINT_D("\r\n rsi_ble_get_profile_async procedure is already in progress -m1 \r\n");
#endif
                  break;
                }
                //! check for buffer full error, which is not expected for this procedure
      ...
      case RSI_BLE_GATT_PROFILE: {
        ...
        if (prof_evt_cnt < total_remote_profiles) {
          //! clear the served event
          rsi_ble_master1_app_clear_event(RSI_BLE_GATT_PROFILE);
          //! copy to master buffer
          memcpy(&rsi_ble_profile_list_by_conn[prof_evt_cnt].profile_info_uuid,
                 &rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].get_profile,
            ...
            if (profile_index_for_char_query < total_remote_profiles) {
...
#if RSI_DEBUG_EN
              LOG_PRINT_D(
                "\r\n search for profile characteristics :0x%x -m1 \r\n",
              ...
              if (status != RSI_SUCCESS) {
                ...
                if (status == RSI_ERROR_BLE_ATT_CMD_IN_PROGRESS) {
                  ...
                  master1_state |= BIT64(RSI_BLE_GATT_PROFILE);
#if RSI_DEBUG_EN
                  LOG_PRINT_D("\r\n rsi_ble_get_char_services_async procedure is already in progress -m1 \r\n");
#endif
                  break;
                }
                //! check for buffer full error, which is not expected for this procedure
...
#if RSI_DEBUG_EN
        LOG_PRINT_D("\r\n in gatt test:RSI_BLE_GATT_CHAR_SERVICES -m1 \r\n");
#endif
        if (char_for_serv_cnt < total_remote_profiles) {
          rsi_ble_master1_app_clear_event(RSI_BLE_GATT_CHAR_SERVICES);
          ...
          rsi_ble_profile_list_by_conn[char_for_serv_cnt].no_of_char_services =
            rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].get_char_services.num_of_services;
          ...
          for (uint8_t ix = 0; ix < rsi_ble_profile_list_by_conn[char_for_serv_cnt].no_of_char_services; ix++) {
            memcpy(&rsi_ble_profile_list_by_conn[char_for_serv_cnt].profile_char_info[ix],
                   &rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].get_char_services,
          ...
          //! discovering completed for all profiles
            ...
            rsi_ble_master1_app_clear_event(RSI_BLE_GATT_CHAR_SERVICES);
            rsi_ble_master1_app_set_event(RSI_BLE_GATT_DESC_SERVICES);
          }
        }
      } break;
...
#if RSI_DEBUG_EN
        LOG_PRINT_D("\r\n in gatt test:RSI_BLE_GATT_DESC_SERVICES : temp1 %d -m1 \r\n", temp1);
#endif
        if (temp1 < total_remote_profiles) {
          temp2++;
          rsi_ble_master1_app_clear_event(RSI_BLE_GATT_DESC_SERVICES);
          char_desc_cnt++;
          char_desc_resp_recvd = false;
        } else {
            ...
            if (rsi_ble_profile_list_by_conn[i].profile_desc.profile_uuid.val.val16 == 0x1802) {
              for (uint8_t ix = 0; ix < rsi_ble_profile_list_by_conn[i].no_of_char_services; ix++) {
                if ((!write_wwr_handle_found)
...
#if RSI_DEBUG_EN
                  LOG_PRINT_D("\n write without response handle found -m1 \n");
#endif
                  ...
                  rsi_ble_master1_app_set_event(RSI_BLE_BUFF_CONF_EVENT);
                  master_1_event_tracker |= BIT(2); //! Trigger Write without response
                  break;
                }
              }
            }
          ...
          if (!write_wwr_handle_found) {
            LOG_PRINT("\n Immediate Alert profile not found -m1 \n");
          }
#endif
        }
      } break;
      case RSI_BLE_BUFF_CONF_EVENT: {
#if RSI_DEBUG_EN
        LOG_PRINT_D("\r\n in gatt RSI_BLE_BUFF_CONF_EVENT -m1 \r\n");
#endif
        ...
        if (status != RSI_SUCCESS) {
          ...
          rsi_ble_master1_app_clear_event(RSI_BLE_BUFF_CONF_EVENT);
          rsi_ble_master1_app_set_event(RSI_CONN_UPDATE_REQ_EVENT);
        }

      } break;
      case RSI_CONN_UPDATE_REQ_EVENT: {
#if RSI_DEBUG_EN
        LOG_PRINT_D("\r\n in connection update req event -m1 \r\n");
#endif
        ...
        if (status != RSI_SUCCESS) {
          ...
          rsi_ble_master1_app_clear_event(RSI_CONN_UPDATE_REQ_EVENT);
          rsi_ble_master1_app_set_event(RSI_DATA_TRANSFER_EVENT);
      ...
      case RSI_BLE_RECEIVE_REMOTE_FEATURES: {
        ...
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_RECEIVE_REMOTE_FEATURES);
#if SMP_ENABLE_M1
        if (master1_mtu_done) {
      ...
      case RSI_BLE_CONN_UPDATE_COMPLETE_EVENT: {
        ...
        rsi_ble_master1_app_clear_event(RSI_BLE_CONN_UPDATE_COMPLETE_EVENT);
        LOG_PRINT("\n conn updated device address : %s\n conn_interval:%d\n supervision timeout:%d -m1",
                  rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].remote_dev_addr,
                  rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].conn_update_resp.conn_interval,
                  rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].conn_update_resp.timeout);
      ...
      case RSI_BLE_DISCONN_EVENT: {
        ...
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_DISCONN_EVENT);

        rsi_ble_master1_app_clear_event(RSI_DATA_TRANSFER_EVENT);

        ble_app_event_task_map[RSI_BLE_MAX_NBR_SLAVES]  = 0;
        ble_app_event_task_map1[RSI_BLE_MAX_NBR_SLAVES] = 0;
#if CHECK_INDICATIONS
        indication_handle       = 0;
        ...
        master_1_event_tracker = 0; //! Clearing all bits

        LOG_PRINT("\r\n master1 is disconnected, reason : 0x%x -m1 \r\n", rsi_m1_discnt_reason);
        ...
        memset(&rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES], 0, sizeof(rsi_ble_conn_info_t));

        if (num_of_conn_masters < RSI_BLE_MAX_NBR_MASTERS) {
          status = rsi_ble_stop_advertising();
          if (status != RSI_SUCCESS) {
            LOG_PRINT("\n advertising failed to stop = 0x%x -m1 \n", status);
            //continue;
          }
          ...
          status = rsi_ble_start_advertising();
          if (status != RSI_SUCCESS) {
            LOG_PRINT("\n advertising failed to start = 0x%x -m1 \n", status);
            //continue;
          }
          LOG_PRINT("\n advertising started -m1 \n");
        }
      } break;
      case RSI_BLE_GATT_WRITE_EVENT: {
        ...
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_GATT_WRITE_EVENT);
#if CHECK_INDICATIONS
        //! Check whether received write is indication and acknowledge it
        if (*(uint16_t *)rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].app_ble_write_event.handle == (indication_handle)) {
        ...
        if ((rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].app_ble_write_event.att_value[0] == NOTIFY_ENABLE)
            && ((*(uint16_t *)(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].app_ble_write_event.handle) - 1)
                == rsi_ble_att1_val_hndl)) {
          LOG_PRINT("\n Remote device enabled the notification -m1\n");
          //! Set flag for notification sending
          master_1_event_tracker |=
            BIT(0); //! BIT 0 for notifications enabled from remote device and sending notifications to it

          //! configure the buffer configuration mode
          rsi_ble_master1_app_set_event(RSI_BLE_BUFF_CONF_EVENT);
        } else if ((rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].app_ble_write_event.att_value[0] == NOTIFY_DISABLE)
                   && ((*(uint16_t *)(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].app_ble_write_event.handle) - 1)
      ...
      case RSI_DATA_TRANSFER_EVENT: {
        ...
        if (status != RSI_SUCCESS) {
          ...
          if (status == RSI_ERROR_BLE_ATT_CMD_IN_PROGRESS) {
            master1_state |= BIT64(RSI_DATA_TRANSFER_EVENT);
#if RSI_DEBUG_EN
            LOG_PRINT_D("\r\n rsi_ble_set_att_value_async procedure is already in progress -m1 \r\n");
#endif
            break;
          } else if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
        ...
        if (status != RSI_SUCCESS) {
          ...
          } else if (status == RSI_ERROR_IN_BUFFER_ALLOCATION) //! TO-DO, add proper error code
          {
            ...
            LOG_PRINT("\r\n write without response failed with status = 0x%x -m1 \r\n", status);
          }
        ...
        if (status != RSI_SUCCESS) {
          if (status == RSI_ERROR_BLE_DEV_BUF_FULL) {
#if RSI_DEBUG_EN
            LOG_PRINT_D("\r\n notify %d failed with buffer full error -m1 \r\n", notfy_cnt);
#endif
            rsi_ble_master1_app_clear_event(RSI_DATA_TRANSFER_EVENT);
            master1_state |= BIT64(RSI_DATA_TRANSFER_EVENT);
            break;
      ...
      case RSI_BLE_WRITE_EVENT_RESP: {
        ...
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_WRITE_EVENT_RESP);
#if CHECK_WRITE_PROPERTY
        rsi_ble_master1_app_set_event(RSI_DATA_TRANSFER_EVENT);
        LOG_PRINT("\n write response received -m1\n");
#endif
      } break;
      case RSI_BLE_READ_REQ_EVENT: {
        ...
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_READ_REQ_EVENT);

        type   = rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].app_ble_read_event.type;
        handle = rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].app_ble_read_event.handle;
        offset = rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].app_ble_read_event.offset;
      ...
      case RSI_BLE_MTU_EVENT: {
        ...
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_MTU_EVENT);
        master1_mtu_done = 1;
#if SMP_ENABLE_M1
        rsi_ble_master1_app_set_event(RSI_BLE_RECEIVE_REMOTE_FEATURES);
#endif
      } break;
      case RSI_BLE_SCAN_RESTART_EVENT: {
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_SCAN_RESTART_EVENT);
      } break;
      case RSI_APP_EVENT_REMOTE_CONN_PARAM_REQ: {

        //!default ACCEPT the remote conn params request (0-ACCEPT, 1-REJECT)
        status =
          rsi_ble_conn_param_resp(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_app_remote_device_conn_params.dev_addr,
                                  1);
        if (status != RSI_SUCCESS) {
        ...
        rsi_ble_master1_app_clear_event(RSI_APP_EVENT_REMOTE_CONN_PARAM_REQ);

      } break;
      case RSI_APP_EVENT_DATA_LENGTH_CHANGE: {
        //! clear the disconnected event.
        rsi_ble_master1_app_clear_event(RSI_APP_EVENT_DATA_LENGTH_CHANGE);

        if (rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].remote_dev_feature.remote_features[1] & 0x01) {
          status = rsi_ble_setphy((int8_t *)rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].remote_dev_feature.dev_addr,
                                  TX_PHY_RATE,
                                  RX_PHY_RATE,
                                  CODDED_PHY_RATE);
          if (status != RSI_SUCCESS) {
      ...
      case RSI_APP_EVENT_PHY_UPDATE_COMPLETE: {
        ...
        rsi_ble_master1_app_clear_event(RSI_APP_EVENT_PHY_UPDATE_COMPLETE);
      } break;

      case RSI_BLE_SMP_REQ_EVENT: {
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_SMP_REQ_EVENT);
        ...
        if (first_connect == 0) {
          //! initiating the SMP pairing process
          status = rsi_ble_smp_pair_request(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_ble_event_smp_req.dev_addr,
          ...
          status = rsi_ble_start_encryption(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_ble_event_smp_req.dev_addr,
                                            rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_encryption_enabled.localediv,
                                            rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_encryption_enabled.localrand,
                                            rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_encryption_enabled.localltk);
        }
#endif
      } break;
      ...
      case RSI_BLE_SMP_RESP_EVENT: {
        ...
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_SMP_RESP_EVENT);

        LOG_PRINT("\r\n in smp response -m1 \r\n");
#if SMP_ENABLE_M1
        //! initiating the SMP pairing process
        status = rsi_ble_smp_pair_response(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_ble_event_smp_resp.dev_addr,
                                           RSI_BLE_SMP_IO_CAPABILITY,
                                           MITM_ENABLE);
#endif
      } break;
      ...
      case RSI_BLE_SMP_PASSKEY_EVENT: {
        ...
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_SMP_PASSKEY_EVENT);
        ...
        //! initiating the SMP pairing process
        status = rsi_ble_smp_passkey(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_ble_event_smp_passkey.dev_addr,
                                     RSI_BLE_APP_SMP_PASSKEY);
      } break;
      case RSI_BLE_SMP_PASSKEY_DISPLAY_EVENT: {
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_SMP_PASSKEY_DISPLAY_EVENT);
#if RSI_DEBUG_EN
        LOG_PRINT_D("\r\n in smp pass key display -m1 \r\n");
#endif

        LOG_PRINT("\nremote addr: %s, passkey: %s \r\n",
                  rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].remote_dev_addr,
                  rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_ble_smp_passkey_display.passkey);
      } break;
      case RSI_BLE_LTK_REQ_EVENT: {
        //! event invokes when disconnection was completed

        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_LTK_REQ_EVENT);

        LOG_PRINT("\r\n in LTK  request -m1 \r\n");

        if (
          0) /* ((rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_le_ltk_resp.localediv == l_rsi_encryption_enabled.localediv)
					&& !((memcmp(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_le_ltk_resp.localrand,
							l_rsi_encryption_enabled.localrand, 8))))*/
        {
          LOG_PRINT("\n positive reply\n");
          //! give le ltk req reply cmd with positive reply
          status = rsi_ble_ltk_req_reply(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_le_ltk_resp.dev_addr,
                                         1,
          ...
          status = rsi_ble_ltk_req_reply(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_le_ltk_resp.dev_addr, 0, NULL);
          if (status != RSI_SUCCESS) {
      ...
      case RSI_BLE_SC_PASSKEY_EVENT: {
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_SC_PASSKEY_EVENT);

        LOG_PRINT("\r\n in smp sc passkey event -m1 \r\n");

        LOG_PRINT("\n In passkey event, remote addr: %s, passkey: %06d -m1 \r\n",
                  rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].remote_dev_addr,
                  rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_event_sc_passkey.passkey);

        rsi_ble_smp_passkey(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_event_sc_passkey.dev_addr,
                            rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_event_sc_passkey.passkey);
      } break;
      ...
      case RSI_BLE_SECURITY_KEYS_EVENT: {
        ...
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_SECURITY_KEYS_EVENT);
      ...
      case RSI_BLE_SMP_FAILED_EVENT: {
        ...
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_SMP_FAILED_EVENT);

        LOG_PRINT("\r\n in smp failed remote address: %s -m1 \r\n",
                  rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].remote_dev_addr);
      }
      ...
      case RSI_BLE_ENCRYPT_STARTED_EVENT: {
        ...
        //! clear the served event
        rsi_ble_master1_app_clear_event(RSI_BLE_ENCRYPT_STARTED_EVENT);
        //! copy to local buffer
        memcpy(&l_rsi_encryption_enabled,
               &rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_encryption_enabled,
               sizeof(rsi_bt_event_encryption_enabled_t));
        LOG_PRINT("\r\n in smp encrypt event -m1 \r\n");
        master1_smp_done = 1;
      } break;
      case RSI_BLE_GATT_ERROR: {
        rsi_ble_master1_app_clear_event(RSI_BLE_GATT_ERROR);
        if ((*(uint16_t *)rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_ble_gatt_err_resp.error)
          ...
          LOG_PRINT("\nGATT ERROR REASON:0x%x -m1 \n",
                    *(uint16_t *)rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES].rsi_ble_gatt_err_resp.error);
        }
