/*******************************************************************************
* @file  rsi_bt_a2dp_source_avrcp_DEMO_54.c
* @brief 
*******************************************************************************
* # License
...
#if (UNIFIED_PROTOCOL && RSI_ENABLE_BT_TEST)
...
#if (INQUIRY_ENABLE || BT_EIR_FRIENDLY_NAME_TEST)
...
uint16_t is_second_pkt_type_set = 0;   //! Considering 2mbps mode set by default
...
/*==============================================*/
/**
 * @fn         rsi_bt_on_ar_stats_event
 * @brief      invoked when AutoRate stats event is received
 * @param[out] ar_stats, auto-rate stats information
 * @return     none.
 * @section description
 * This callback function indicates auto-rate stats details in BT device.
 */
...
int32_t rsi_bt_app_sbc_encode()
{
  ...
  if (!power_save_given) {
    //! enable wlan radio
    err = rsi_wlan_radio_init();
    if (err != RSI_SUCCESS) {
      LOG_PRINT("\n radio init failed \n");
    }
    //! initiating power save in BLE mode
    err = rsi_bt_power_save_profile(PSP_MODE, PSP_TYPE);
    if (err != RSI_SUCCESS) {
      LOG_PRINT("\r\n Failed in initiating power save \r\n");
      //continue;
    }
    //! initiating power save in wlan mode
    err = rsi_wlan_power_save_profile(RSI_SLEEP_MODE_2, PSP_TYPE);
    if (err != RSI_SUCCESS) {
      LOG_PRINT("\r\n Failed in initiating power save \r\n");
      //continue;
    }
...
int32_t rsi_bt_a2dp_source_sbc_codec(void)
{
  ...
  rsi_bt_gap_register_callbacks(NULL, //role_change
...
#if ((RSI_APP_AVDTP_ROLE != ACCEPTOR_ROLE) && (INQUIRY_ENABLE || BT_EIR_FRIENDLY_NAME_TEST))
                                ...
                                NULL,
                                rsi_bt_app_on_connection_initiated);
  ...
  //! Start inquiry
  status = rsi_bt_inquiry(2, INQUIRY_DURATION, MAX_NO_OF_RESPONSES);
  if (status != 0) {
...
int32_t rsi_bt_app_task()
{
  ...
  if (status != RSI_SUCCESS) {
    LOG_PRINT("bt_a2dp init failed with status %x\n", status);
    return status;
  }
  while (1)
#endif
  {
    ...
    switch (temp_event_map) {
      ...
      case RSI_APP_EVENT_CONNECT: {
        ...
        for (uint8_t i = 0; i < inq_responses_count; i++) {
          //! check for remotedevice1 name
          if (!device1_found) {
            if (strcmp((const char *)&l_resp_event[i].remote_device_name, RSI_BT_REMOTE_DEV1_NAME) == 0) {
              device1_found = true;
            }
          }
          //! check for remotedevice2 name
          if (!device2_found) {
            if (strcmp((const char *)&l_resp_event[i].remote_device_name, RSI_BT_REMOTE_DEV2_NAME) == 0) {
              device2_found = true;
            }
          }
        }
        if (device1_found && device2_found) {
          //! stop the inquiry and calculate time
          end_time   = rsi_hal_gettickcount();
          total_time = end_time - start_time;
          LOG_PRINT("Time taken in msec to resolve friendly names : %d\n", total_time);
        } else {
          //! start inquiry again
          inq_responses_count = 0;
          //! Start inquiry
      ...
      case RSI_APP_EVENT_REMOTE_NAME_REQ: {
        ...
        for (inq_resp_name_length_index; inq_resp_name_length_index < inq_responses_count;
             inq_resp_name_length_index++) {
...
#if (RSI_APP_AVDTP_ROLE != ACCEPTOR_ROLE)
#if CHECK_BT_POWER_STATE
        LOG_PRINT("\n In deepsleep after disconnection for 30sec \n");
#else
        LOG_PRINT("\n looking for device\n");
#if (RSI_BT_DUAL_PAIR_TEST == 1)
        if (dual_pair_flag == 0) {
