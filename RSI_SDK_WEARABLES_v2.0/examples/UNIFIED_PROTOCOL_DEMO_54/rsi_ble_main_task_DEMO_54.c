/*******************************************************************************
* @file  rsi_ble_main_task_DEMO_54.c
* @brief 
*******************************************************************************
* # License
...
uint16_t mtu_size; //! TODO -- can be removed?
uint16_t rsi_scan_in_progress;
uint32_t ble_main_app_event_task_map;
uint32_t ble_app_event_task_map[RSI_BLE_MAX_NBR_MASTERS + RSI_BLE_MAX_NBR_SLAVES];
uint32_t ble_app_event_task_map1[RSI_BLE_MAX_NBR_MASTERS + RSI_BLE_MAX_NBR_SLAVES];
...
volatile uint16_t rsi_s1_discnt_reason = 0, rsi_s2_discnt_reason = 0, rsi_s3_discnt_reason = 0,
                  rsi_m1_discnt_reason = 0, rsi_m2_discnt_reason = 0;
...
extern uint8_t slave_1_received_notifications;
extern uint8_t slave_2_received_notifications;
extern uint8_t slave_3_received_notifications;
extern uint8_t slave_1_event_tracker;
extern uint8_t slave_2_event_tracker;
extern uint8_t slave_3_event_tracker;
...
//   ! EXTERN FUNCTIONS
/*=======================================================================*/
extern int32_t rsi_ble_set_local_irk_value(uint8_t *l_irk);
/*=======================================================================*/
//   ! PROCEDURES
/*=======================================================================*/
...
void rsi_ble_app_set_task_event(uint8_t conn_id, uint32_t event_num)
{
  ...
  switch (conn_id) {
    case 0:
      ...
      rsi_semaphore_post(&ble_slave1_sem);
      break;
    case 1:
      if (event_num < 32) {
        ble_app_event_task_map[conn_id] |= BIT(event_num);
      ...
      rsi_semaphore_post(&ble_slave2_sem);
      break;
    case 2:
      if (event_num < 32) {
        ble_app_event_task_map[conn_id] |= BIT(event_num);
      ...
      rsi_semaphore_post(&ble_slave3_sem);
      break;
    case 3:
      if (event_num < 32) {
        ble_app_event_task_map[conn_id] |= BIT(event_num);
      ...
      rsi_semaphore_post(&ble_master1_sem);
      break;
    case 4:
      if (event_num < 32) {
        ble_app_event_task_map[conn_id] |= BIT(event_num);
      ...
      rsi_semaphore_post(&ble_master2_sem);
      break;
    default:
      break;
  }
...
static uint32_t rsi_ble_add_simple_chat_serv(void)
{
  ...
  rsi_ble_add_char_serv_att(
    ...
    RSI_BLE_ATT_PROPERTY_READ | RSI_BLE_ATT_PROPERTY_NOTIFY | RSI_BLE_ATT_PROPERTY_WRITE_NO_RESPONSE,
...
void rsi_ble_simple_central_on_adv_report_event(rsi_ble_event_adv_report_t *adv_report)
{
  ...
  if (device_already_connected == SLAVE_NOT_CONNECTED) {
    ...
    //! set conn specific event
    rsi_ble_app_set_task_event(ble_conn_id, RSI_APP_EVENT_ADV_REPORT);
...
static void rsi_ble_on_connect_event(rsi_ble_event_conn_status_t *resp_conn)
{
  ...
  if (remote_device_role == SLAVE_ROLE) //check for the connection is from slave or master
  {
    ...
    memcpy(rsi_ble_conn_info[ble_conn_id].remote_dev_addr, remote_dev_addr_conn, sizeof(remote_dev_addr_conn));
    //! copy to conn specific buffer
    ...
    for (i = 0; i < (RSI_BLE_MAX_NBR_MASTERS); i++) {
      if (!memcmp(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES + i].remote_dev_addr,
        ...
        rsi_ble_app_set_task_event(RSI_BLE_MAX_NBR_SLAVES + i, RSI_BLE_CONN_EVENT);
        break;
      }
    }
  } else {
...
static void rsi_ble_on_disconnect_event(rsi_ble_event_disconnect_t *resp_disconnect, uint16_t reason)
{
  ...
  if (ble_conn_id == 0) {
    rsi_s1_discnt_reason = reason;
  } else if (ble_conn_id == 1) {
    rsi_s2_discnt_reason = reason;
  } else if (ble_conn_id == 2) {
    rsi_s3_discnt_reason = reason;
  } else if (ble_conn_id == 3) {
    rsi_m1_discnt_reason = reason;
  } else if (ble_conn_id == 4) {
    rsi_m2_discnt_reason = reason;
  }
  //! set conn specific event
  ...
  if (remote_device_role == SLAVE_ROLE) //check for the connection is from slave or master
  {
    ...
    for (i = 0; i < (RSI_BLE_MAX_NBR_MASTERS); i++) {
      if (!memcmp(rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES + i].remote_dev_addr,
        ...
        rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES + i].conn_id            = RSI_BLE_MAX_NBR_SLAVES + i;
        rsi_ble_conn_info[RSI_BLE_MAX_NBR_SLAVES + i].remote_device_role = MASTER_ROLE;
        rsi_ble_app_set_task_event(RSI_BLE_MAX_NBR_SLAVES + i, RSI_BLE_ENHC_CONN_EVENT);
        break;
      }
    }
  } else {
...
static void rsi_ble_on_gatt_write_event(uint16_t event_id, rsi_ble_event_write_t *rsi_ble_write)
{
  ...
  //! if notify is happen
  if (ble_conn_id == 0) {
    slave_event_tracker = slave_1_event_tracker;
  } else if (ble_conn_id == 1) {
    slave_event_tracker = slave_2_event_tracker;
  } else if (ble_conn_id == 2) {
    slave_event_tracker = slave_3_event_tracker;
  }
  //! For slave 0, receiving notifications
  if ((slave_event_tracker & BIT(1))) {
    if (ble_conn_id == 0) {
      slave_1_received_notifications++;
      if (slave_1_received_notifications != 3) {
#if RSI_DEBUG_EN
        LOG_PRINT_D("\n receiving notifications %d -s1 \n", slave_1_received_notifications);
#endif
        return;
      }
    } else if (ble_conn_id == 1) {
      slave_2_received_notifications++;
      if (slave_2_received_notifications != 3) {
#if RSI_DEBUG_EN
        LOG_PRINT_D("\n receiving notifications %d -s2 \n", slave_1_received_notifications);
#endif
        return;
      }
    } else if (ble_conn_id == 2) {
      slave_3_received_notifications++;
      if (slave_3_received_notifications != 3) {
#if RSI_DEBUG_EN
        LOG_PRINT_D("\n receiving notifications %d -s3 \n", slave_1_received_notifications);
#endif
        return;
      }
    }
  } else if ((slave_event_tracker & BIT(2))) //! receiving any write
  {
    return;
  }
  //! copy to conn specific buffer
 ...
 */
static void rsi_ble_data_length_change_event(
		rsi_ble_event_data_length_update_t *rsi_ble_data_length_update) {

	//! convert to ascii
	rsi_6byte_dev_address_to_ascii(remote_dev_addr_conn, rsi_ble_data_length_update->dev_addr);
	...
	//! set conn specific event
	//! signal conn specific task
	rsi_ble_app_set_task_event(ble_conn_id, RSI_APP_EVENT_DATA_LENGTH_CHANGE);
}
#endif
/*==============================================*/
/**
...
static void rsi_ble_on_smp_failed(uint16_t status, rsi_bt_event_smp_failed_t *remote_dev_address)
{
  ...
  LOG_PRINT("\n SMP failed for remote address : %s with status : %x", remote_dev_address, status);
...
static int32_t rsi_ble_dual_role(void)
{
  ...
  change_adv_param.adv_type         = RSI_BLE_ADV_TYPE;
  change_adv_param.filter_type      = RSI_BLE_ADV_FILTER_TYPE;
  change_adv_param.direct_addr_type = RSI_BLE_ADV_DIR_ADDR_TYPE;
  change_adv_param.adv_int_min      = RSI_BLE_ADV_INT_MIN_AFTER;
  change_adv_param.adv_int_max      = RSI_BLE_ADV_INT_MAX_AFTER;
  ...
  change_scan_param.scan_type     = RSI_BLE_SCAN_TYPE;
  change_scan_param.filter_type   = RSI_BLE_SCAN_FILTER_TYPE;
  change_scan_param.scan_int      = LE_SCAN_INTERVAL_AFTER; //! scan interval 33.125ms
  change_scan_param.scan_win      = LE_SCAN_WINDOW_AFTER;   //! scan window 13.375ms
