/*******************************************************************************
* @file  rsi_ble_main_task_DEMO_69.c
* @brief 
*******************************************************************************
* # License
...
#include <rsi_wlan_sta_tx_config_DEMO_69.h>
...
uint8_t rsi_check_dev_list(uint8_t *remote_dev_name, uint8_t *adv_dev_addr)
{
  ...
  if (!strcmp(RSI_BLE_DEV_1_ADDR, (char *)advertised_dev_addr)) {
    ...
    slave_device_found = SLAVE_FOUND;
  } else if (!strcmp(RSI_BLE_DEV_5_ADDR, (char *)advertised_dev_addr)) {
    slave_device_found = SLAVE_FOUND;
  } else if (!strcmp(RSI_BLE_DEV_6_ADDR, (char *)advertised_dev_addr)) {
    slave_device_found = SLAVE_FOUND;
  } else if (!strcmp(RSI_BLE_DEV_7_ADDR, (char *)advertised_dev_addr)) {
    slave_device_found = SLAVE_FOUND;
  } else if (!strcmp(RSI_BLE_DEV_8_ADDR, (char *)advertised_dev_addr)) {
    slave_device_found = SLAVE_FOUND;
  } else
    return slave_device_found;
